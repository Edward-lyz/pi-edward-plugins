...
Install from the Git source repository. The install script installs npm dependencies
(including the `fff` extension's `@ff-labs/fff-node` native module), ensures the
`rtk` CLI is present, and registers the source checkout with pi:
...
| `subagent` | `packages/subagent/src/index.ts` | Runs delegated work in SDK-created in-memory Pi subagent sessions with tintinweb-style live widget and FleetView. | `/subagent` opens a model picker over currently imported Pi models, then a thinking-level picker; `/subagent status` prints defaults; tool args are `prompt`, optional `model`, optional `thinking`, optional `run_in_background`; use `wait_subagent` to wait for background results; child sessions filter out subagent tools to prevent recursion. |
...
| `usage-report` | `packages/usage-report/src/usage-report.ts` | Serves a local usage dashboard with token activity, tool/skill counts, and API-cost estimates. | `/usage-report` opens the dashboard by default; use `/usage-report status\|start [port]\|stop` for service control; model prices come from Pi model config plus LiteLLM's public price table. |
