...
export function getDisplayName(_type: SubagentType): string {
  return "Subagent";
}
...
export class AgentWidget {
   ...
   *     (`isBackground === false`); keep everything else — background
   *     or external spawns (`undefined`). Keying off the `isBackground`
  ...
  private renderWidget(tui: any, theme: Theme): string[] {
    ...
    const finished = allAgents.filter(a =>
      a.status !== "running" && a.completedAt
    ...
    if (totalBody <= maxBody) {
      ...
      // Overflow — prioritize: running > finished.
  ...
  update() {
    ...
    if (hasActive) {
      newStatusText = `${runningCount} running agent${runningCount === 1 ? "" : "s"}`;
    }
