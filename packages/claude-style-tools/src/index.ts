...
function getToolArgSummary(tool: any): string {
	...
	if (name === "exec_command") return summarizeCodexExecCommand(args);
	if (name === "write_stdin") return summarizeCodexStdinCall(args);
	if (name === "apply_patch") return summarizeCodexApplyPatchCall(args, (path: string) => shortPath(process.cwd(), path));
...
function getApplyPatchText(args: any): string {
	return getStringArg(args, "patchText", "patch_text", "input", "patch", "diff");
}
...
function summarizeCodexApplyPatchCall(args: any, sp: (path: string) => string): string {
	const patchText = getApplyPatchText(args);
	const files = extractApplyPatchFiles(patchText);
	if (files.length === 0) return "patch";
	if (files.length === 1) return sp(files[0]);
	return `${sp(files[0])} (+${files.length - 1} files)`;
}

function summarizeCodexExecCommand(args: any): string {
	const command = getStringArg(args, "cmd", "command");
	const summary = summarizeText(command || "shell command", 72);
	const workdir = getStringArg(args, "workdir", "cwd");
	return workdir ? `${summary} @ ${shortPath(process.cwd(), workdir)}` : summary;
}

function summarizeCodexStdinCall(args: any): string {
	const sessionId = args?.session_id ?? args?.sessionId;
	const prefix = sessionId === undefined ? "session" : `session ${sessionId}`;
	if (typeof args?.chars !== "string") return `poll ${prefix}`;
	if (args.chars.length === 0) return `poll ${prefix}`;
	if (args.chars === "\u0003") return `interrupt ${prefix}`;
	if (args.chars === "\u0004") return `close stdin ${prefix}`;
	return `send ${summarizeText(JSON.stringify(args.chars), 48)} to ${prefix}`;
}
...
function getApplyPatchResultMeta(args: any, ctx: any, sp: (path: string) => string): ApplyPatchResultMeta | null {
	const patchText = getApplyPatchText(args ?? ctx?.args);
	if (!patchText) return null;
...
function summarizeOpenAiToolCall(name: string, args: any, theme: Theme, sp: (path: string) => string): string {
	switch (name) {
		case "apply_patch": {
			const summary = summarizeCodexApplyPatchCall(args, sp);
			return summary === "patch" ? theme.fg("muted", summary) : summary.replace(/\(\+\d+ files\)$/, (suffix) => theme.fg("muted", suffix));
		}
		case "exec_command":
			return summarizeCodexExecCommand(args);
		case "write_stdin":
			return summarizeCodexStdinCall(args);
		case "webfetch":
...
function getCodexShellOutput(result: any): string {
	if (typeof result?.details?.output === "string") return normalizeToLf(result.details.output).trimEnd();
	const raw = normalizeToLf(getTextContent(result));
	const marker = "\nOutput:\n";
	const outputStart = raw.indexOf(marker);
	return (outputStart === -1 ? raw : raw.slice(outputStart + marker.length)).trimEnd();
}

function getCodexShellStatus(raw: string, result: any): { running: boolean; exitCode: number | null; sessionId: string } {
	const runningMatch = raw.match(/Process running with session ID (\d+)/);
	const exitMatch = raw.match(/Process exited with code (-?\d+)/);
	const detailsSessionId = result?.details?.session_id ?? result?.details?.sessionId;
	return {
		running: Boolean(runningMatch),
		exitCode: exitMatch ? Number.parseInt(exitMatch[1], 10) : null,
		sessionId: runningMatch?.[1] ?? (detailsSessionId === undefined ? "" : String(detailsSessionId)),
	};
}

function renderCodexShellResult(name: string, result: any, expanded: boolean, isPartial: boolean, theme: Theme, ctx: any): Text {
	const raw = normalizeToLf(getTextContent(result));
	const output = getCodexShellOutput(result);
	const lines = output ? output.split("\n").filter((line) => line.trim().length > 0) : [];
	const status = getCodexShellStatus(raw, result);

	if (isPartial) {
		setToolStatus(ctx, "pending");
		const session = status.sessionId ? ` session ${status.sessionId}` : "";
		return makeText(ctx.lastComponent, runningPreviewBlock(result, theme.fg("warning", `Running${session}...`), expanded, theme, ctx, {
			lines,
	...
	if (status.running) {
		...
		setToolStatus(ctx, "pending");
		const session = status.sessionId ? ` session ${status.sessionId}` : "";
		let text = theme.fg("warning", `Running${session}`);
		if (lines.length > 0) text += theme.fg("muted", ` (${lineCountLabel(lines.length)})`);
		if (!expanded || lines.length === 0) return makeText(ctx.lastComponent, withBranch(`${text}${lines.length > 0 ? toolOutputDetailHint(theme, expanded) : ""}`, theme));
		const preview = buildPreviewText(lines.map((line) => theme.fg("dim", line || " ")), false, theme, bashCollapsedLimit());
		return makeText(ctx.lastComponent, withBranch(`${text}\n${preview}`, theme));
	}

	clearBlinkTimer(ctx);
	const failed = ctx.isError || (status.exitCode !== null && status.exitCode !== 0);
	setToolStatus(ctx, failed ? "error" : "success");

	if (ctx.isError) {
		const firstLine = raw.trim().split("\n").find((line) => line.trim()) ?? `${humanizeToolName(name)} failed`;
		return makeText(ctx.lastComponent, withBranch(theme.fg("error", firstLine), theme));
	}

	const statusText = status.exitCode === null || status.exitCode === 0
		? theme.fg("success", "Done")
		: theme.fg("error", `Exit ${status.exitCode}`);
	const lineLabel = lines.length > 0 ? theme.fg("muted", ` (${lineCountLabel(lines.length)})`) : "";
	const text = `${statusText}${lineLabel}`;
	if (!expanded || lines.length === 0) return makeText(ctx.lastComponent, withBranch(`${text}${lines.length > 0 ? toolOutputDetailHint(theme, expanded) : ""}`, theme));
	const preview = buildPreviewText(lines.map((line) => theme.fg("dim", line || " ")), false, theme, bashCollapsedLimit());
	return makeText(ctx.lastComponent, withBranch(`${text}\n${preview}`, theme));
}
...
function renderOpenAiToolResult(name: string, result: any, expanded: boolean, isPartial: boolean, theme: Theme, ctx: any): Text {
	if (name === "exec_command" || name === "write_stdin") {
		return renderCodexShellResult(name, result, expanded, isPartial, theme, ctx);
	}
...
export default function (pi: ExtensionAPI) {
	...
	const registerOpenAiToolOverrides = (): void => {
		...
		for (const tool of allTools) {
			...
			const label = name === "exec_command" || name === "write_stdin" ? genericToolLabel(name) : rawLabel && rawLabel !== name && !rawLabel.includes("_") ? rawLabel : humanizeToolName(name);
