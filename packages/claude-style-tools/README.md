# pi-claude-style-tools-codex

Vendored fork of `pi-claude-style-tools` for `pi-better-ux`.

Local changes:

- render Codex `exec_command` as Claude-style `Bash` rows
- render Codex `write_stdin` as interactive stdin / polling rows
- accept Codex `apply_patch` payloads under `input`, in addition to the upstream `patchText` / `patch_text` fields

The original upstream package is `pi-claude-style-tools` by FammasMaz.
